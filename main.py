...
def foyer():
    print("Tu ieej foajē. Ir tumšs, bet redzi durvis uz 'virtuvi', 'dzīvojamo istabu', 'pagrabu', 'bibliotēku' un 'tualeti'.")
    choice = ""
    while choice not in ["virtuve", "dzīvojamā istaba", "pagrabs", "bibliotēka", "tualete"]:
        choice = input(">>> ").lower()
        if choice == "virtuve":
            kitchen()
...
def library():
    print("Tu esi bibliotēkā. Te ir daudz grāmatu, bet viena no tām ir nolādēta. Vai tu vēlies 'lasīt' grāmatu vai iet 'atpakaļ'?")
    choice = ""
    while choice not in ["lasīt", "atpakaļ"]:
        choice = input(">>> ").lower()
        if choice == "lasīt":
            print("Tu izlasīji nolādētu grāmatu un tagad jūties ļoti noguris. Spēle beigusies.")
            end_game()
        elif choice == "atpakaļ":
            foyer()
        else:
...
def toilet():
    print("Tu esi tualetē. Te ir aizdomīga spogulīte. Vai tu vēlies 'skatīties' spogulī vai iet 'atpakaļ'?")
    choice = ""
    while choice not in ["skatīties", "atpakaļ"]:
        choice = input(">>> ").lower()
        if choice == "skatīties":
            print("Tu ieraugi savu spoku attēlu! Spēle beigusies.")
            end_game()
        elif choice == "atpakaļ":
            foyer()
        else:
